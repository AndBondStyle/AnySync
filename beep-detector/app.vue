<template>
    <div class="flex-col">
        <div class="flex-row">
            <p>FFT CHUNK SIZE:</p>
            <v-button-switch v-model="fftSize" @input="setupDetector" :items="[512, 1024, 2048, 4096]"></v-button-switch>
        </div>
        <div class="flex-row">
            <div class="flex-col">
                <div class="flex-row">
                ...
                <div class="flex-row">
                    <p>MIN VOLUME:</p>
                    <v-spinbox v-model="minVolume" :min="-100" :max="maxVolume">dB</v-spinbox>
                </div>
                <div class="flex-row">
                    <p>MAX VOLUME:</p>
                    <v-spinbox v-model="maxVolume" :min="minVolume" :max="-30">dB</v-spinbox>
                </div>
                <div class="flex-row">
                    <p>DET VOLUME:</p>
    ...
    export default {
        ...
        methods: {
            ...
            setupDetector() {
                ...
                this.detector = new Detector(this.context, this.source, this);
                this.detector.onchunk = (v, b) => { this.value = v; this.bars = b };
            }
        }
    }
</script>
